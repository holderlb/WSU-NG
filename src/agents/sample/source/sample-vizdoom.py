...
from random_action import Agent
...
class TA2Agent(TA2Logic):
    def __init__(self):
        ...
        available_actions = [{'action': 'nothing'},
                             {'action': 'left'},
                             {'action': 'right'},
                             {'action': 'backward'},
                             {'action': 'forward'},
        ...
        self.agent = Agent(available_actions)
    ...
    def training_instance(self, feature_vector: dict, feature_label: dict) -> dict:
        ...
        label_prediction = self.agent.train_inst(feature_vector, feature_label)
    ...
    def train_model(self):
        ...
        self.log.info('Simulating training with a 2 second sleep.')
        time.sleep(2)
        self.agent.train()
    ...
    def trial_start(self, trial_number: int, novelty_description: dict):
        ...
        """
        self.log.info('Trial Start: #{}  novelty_desc: {}'.format(trial_number, str(novelty_description)))
        return
